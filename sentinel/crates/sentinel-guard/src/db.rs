...
//!
//! Par défaut le garde partage la base de l'app desktop
//! (`<app-data>/com.sentinel-mcp.desktop/sentinel.db`). `--db` permet
//! d'en cibler une autre. Même convention que sentinel-cli (qui est un
//! binaire pur — la fonction ne peut pas être importée de là).
...
/// les répertoires parents si nécessaire.
...
mod tests {
    ...
    fn chemin_par_defaut_pointe_vers_la_base_desktop() {
        ...
        assert!(
            chemin.ends_with("com.sentinel-mcp.desktop/sentinel.db")
                || chemin.ends_with("sentinel.db")
        );
    }
